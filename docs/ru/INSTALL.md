...
Если ставишь `xrpl-cloud`, cloud-сервер (`xrpl.mcp.staticbit.ai`) защищён **OAuth 2.1** против `auth.mcp.staticbit.ai`. **Никакого bearer-токена и ENV-переменной** задавать не нужно — вместо этого твой аккаунт должен быть в **allow-list** сервера. Попроси админа `xrpl.mcp.staticbit.ai` добавить тебя.

Как только ты в allow-list, вход выполняется интерактивно из Claude Code через `/mcp` (см. §9) — браузерный flow проведёт тебя через `auth.mcp.staticbit.ai`, Claude Code сделает dynamic client registration, сохранит полученный токен и будет обновлять его автоматически. Ничего копировать или вставлять не нужно, в менеджере паролей хранить нечего.
