...
- **SourceTag attribution** — every `*_prepare`d transaction is stamped with `SourceTag` `100010011` (StaticBit's MCP tag, sibling of the platform tag `100010010`) unless the caller supplied one; an explicit SourceTag — including `0` — is preserved, and for an XLS-56 Batch only the outer transaction is tagged. Override or disable via `StaticBitXrplMcp__DefaultSourceTag`.
- **Transaction preview** — every `*_prepare` returns a full-disclosure `preview` block (full un-truncated addresses, drops→XRP, an anomalous-fee flag, `LastLedgerSequence` expiry estimate, decoded **untrusted** memos) rendered by `TransactionPreview` for the user-approval prompt.
