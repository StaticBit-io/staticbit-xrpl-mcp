...
Prerequisites: `xrpl-local` (prepare) **and** `xrpl-signer` (offline keystore) plugins installed, and a
funded payer wallet alias in the signer. HTTP is done by the agent with `curl` (full header control);
no outbound HTTP happens inside the MCP.

Tool prefixes:
- prepare / decode: `mcp__plugin_xrpl-local_xrpl-local__xrpl_*`
...
5. **Prepare** (autofill + canonical encode + preview):
   `mcp__plugin_xrpl-local_xrpl-local__xrpl_tx_prepare_generic(network=$NETNAME, txJson=$TX, humanSummary="x402 payment to <payTo> for <invoiceId>")`
...
- **Prepare / decode / lookup** are owned by `xrpl-local-operations` (this plugin). Use the same
  `xrpl-local` server for the whole flow.
...
- A plain XRPL payment with no HTTP 402 involved — use `xrpl-local-operations` (`xrpl_payment_prepare`).
